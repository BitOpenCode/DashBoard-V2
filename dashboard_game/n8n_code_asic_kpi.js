// Код для CODE ноды в n8n для форматирования данных ASIC KPI
// Входные данные: результат SQL запроса из Postgres ноды
// SQL запрос возвращает пользователей, которым не хватает 1 ASIC для перехода на следующий уровень
...
const formattedUsers = usersList.map((user, index) => {
  ...
  const missingAsics = typeof user.missing_asics === 'string' 
    ? parseInt(user.missing_asics, 10) || 0 
    : parseInt(user.missing_asics) || 0;
  
  const progressPercent = typeof user.progress_percent === 'string' 
    ? parseFloat(user.progress_percent) || 0 
    : parseFloat(user.progress_percent) || 0;
  ...
  const requiredAsics = typeof user.required_asics_for_next_level === 'string' 
    ? parseInt(user.required_asics_for_next_level, 10) || null 
    : parseInt(user.required_asics_for_next_level) || null;
  ...
  return {
    ...
    required_asics_for_next_level: requiredAsics,
    missing_asics: missingAsics,
