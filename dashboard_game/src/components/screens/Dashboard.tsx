...
import { 
  RefreshCw, Users, TrendingUp, Search, ChevronRight, X, Activity, Wallet, Globe, Calendar
} from 'lucide-react';
import { EventIcon } from '../dashboard/EventIcon';
import { numberFormat, formatFullNumber } from '../../utils/dashboard/formatters';
import { hexToUserFriendlyAddress, getTonUsdRate, checkWalletBalance as checkWalletBalanceUtil } from '../../utils/dashboard/tonUtils';
import { 
  useUsersData, 
  useWalletsData, 
  useEventsData, 
...
// Утилиты форматирования вынесены в отдельные файлы
...
const Dashboard: React.FC = () => {
  ...
  // Используем хук для событий
  const {
    eventsData,
    loading: eventsLoading,
    timeFilter: eventsTimeFilter,
    setTimeFilter: setEventsTimeFilter,
    loadEventsData,
    setEventsData,
  } = useEventsData();
  ...
  // Используем хук для KPI
  const {
    kpiData,
    loading: kpiLoading,
    selectedLevel: selectedKpiLevel,
    setSelectedLevel: setSelectedKpiLevel,
    loadKpiData,
    setKpiData,
  } = useKPIData();
  ...
  // Обертки для функций загрузки с логикой скрытия других секций
  ...
  const handleLoadEventsData = async () => {
    ...
    // Загружаем данные TON заказов после загрузки событий
  ...
  // Функции loadReferralsData, loadFunnelData, loadLeadersData, loadPoolsData, loadKpiData вынесены в хуки
  ...
  // Функция loadKpiData вынесена в хук useKPIData
  ...
  // Функция для конвертации HEX адреса вынесена в утилиты

  // Функция для проверки баланса TON кошелька (обертка над утилитой)
  ...
  // Фильтрация и группировка данных по периоду вынесена в UsersSection

  // Фильтрованные данные events по времени - перемещены в EventsSection
  ...
  // Инициализация выбранных событий перемещена в EventsSection
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6 md:max-w-4xl">
      ...
      <div className="mb-8 neu-btn-grid">
        <button onClick={handleLoadUsersData} disabled={usersLoading} className="neu-btn-lg">
          {usersLoading ? (
        ...
        <button onClick={handleLoadWalletsData} disabled={walletsLoading} className="neu-btn-lg">
          {walletsLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={handleLoadEventsData} disabled={eventsLoading} className="neu-btn-lg">
          {eventsLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={handleLoadReferralsData} disabled={referralsLoading} className="neu-btn-lg">
          {referralsLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={handleLoadPoolsData} disabled={poolsLoading} className="neu-btn-lg">
          {poolsLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={handleLoadFunnelData} disabled={funnelLoading} className="neu-btn-lg">
          {funnelLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={handleLoadLeadersData} disabled={leadersLoading} className="neu-btn-lg">
          {leadersLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={handleLoadKpiData} disabled={kpiLoading} className="neu-btn-lg">
          {kpiLoading ? (
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ) : (
        ...
        <button onClick={loadAllUsersData} disabled={allUsersLoading} className="neu-btn-lg">
          ...
          <span>{allUsersLoading ? 'Loading...' : 'All Users'}</span>
        </button>
      </div>

      {/* Отображение данных пользователей с кошельками */}
      ...
      {usersData && <UsersSection usersData={usersData} timeFilter={timeFilter} setTimeFilter={setTimeFilter} />}

      {/* Отображение данных кошельков */}
      {walletsData && <WalletsSection walletsData={walletsData} />}

      {/* Отображение данных рефералов */}
      {referralsData && <ReferralsSection referralsData={referralsData} />}
      ...
      <EventsSection
        ...
        onSelectEventModal={setSelectedEventModal}
      ...
      {funnelData && <FunnelSection funnelData={funnelData} onLoadLevelUsers={loadLevelUsers} />}

      {/* Отображение таблицы лидеров */}
      {leadersData && <LeadersSection leadersData={leadersData} />}

      {/* Отображение топ-3 пулов */}
      {poolsData && <PoolsSection poolsData={poolsData} />}
